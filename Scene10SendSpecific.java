...
public class Scene10SendSpecific {

    @FXML
    private TextField topicField;

    @FXML
    private TextArea specificMessageField;
    ...
    public void initialize() {
        ...
        topicField.textProperty().addListener((observable, oldValue, newValue) -> validateForm());
        specificMessageField.textProperty().addListener((observable, oldValue, newValue) -> validateForm());
    }

    private void validateForm() {
        // Enable the submit button only if both fields are not empty
        boolean formValid = !topicField.getText().trim().isEmpty() &&
                            !specificMessageField.getText().trim().isEmpty();
        submitButton.setDisable(!formValid);
    }

    @FXML
    private void handleSubmit(ActionEvent event) {
        // Print topic and message to the console
